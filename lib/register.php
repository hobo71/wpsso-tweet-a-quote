...
if ( ! class_exists( 'WpssoTaqRegister' ) ) {

	class WpssoTaqRegister {

		public function __construct() {

			register_activation_hook( WPSSOTAQ_FILEPATH, array( $this, 'network_activate' ) );

			//register_deactivation_hook( WPSSOTAQ_FILEPATH, array( $this, 'network_deactivate' ) );
		...
		private function activate_plugin() {
			...
			if ( class_exists( 'Wpsso' ) ) {
				...
				if ( class_exists( 'WpssoUtil' ) ) {	// Just in case.
					...
					$version = WpssoTaqConfig::$cf[ 'plugin' ][ 'wpssotaq' ][ 'version' ];

					WpssoUtil::save_all_times( 'wpssotaq', $version );
				}

			} else {
				WpssoTaq::required_notice( $deactivate = true );
			}
		}
